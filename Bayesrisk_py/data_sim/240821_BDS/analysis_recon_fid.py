...
lims = [[-0.02, 0.27], [-0.02, 0.652], [-0.02, 0.452], [-0.02, 0.452]]
...
for j in range(len(meas)):
    for i in range(len(recon)):
        temp = fid[i][j][:len(n_meas[j])]
        temp_std = np.std(1 - temp, axis=1) / np.sqrt(n_sample)
        if not(i > 1 and j > 3):
            axs[*pos[j]].errorbar(n_meas[j], np.average(temp, axis=1), yerr= temp_std, c= c_meas[j], lw=l_w, ls= "", marker= markers[i], ms= m_s, label= recon[i], alpha=1, zorder= 1)

    axs[*pos[j]].set_title(meas[j])
    axs[*pos[j]].set_xlim(0, 93)
    axs[*pos[j]].set_ylim(*lims[0])
...
axs[0, 0].set_ylabel(r'average risk (1-F)', fontsize=f_s)
axs[1, 0].set_ylabel(r'average risk (1-F)', fontsize=f_s)

plt.savefig("BDS_recon_fid", dpi= 600)
plt.show()