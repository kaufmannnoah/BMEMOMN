...
meas = ['bell']
dim = [4]
n_meas = np.array([4, 8, 12, 16, 20, 24, 28, 32, 36, 40])
n_sample = 1000
...
for j in range(len(recon)):
    ...
    axs[1].plot(x, 3/(5*(4+x)), c= c_meas[0], ls= ":")
    axs[1].plot(x, 3/(5*x), c= c_meas[2], ls= ":")
...
plt.savefig("BDS_BellMeas_Analysis", dpi= 600)
plt.show()