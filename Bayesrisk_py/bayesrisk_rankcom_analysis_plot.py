...
imp_data = np.load("data/output_bayesrisk_rankcomp_240621.npy")
...
#fit exponential
coef = np.zeros((len(nq), 3, 2))
coef_std = np.zeros((len(nq), 3, 2))
for j in range(len(nq)):
    for i in range(3):
        coef[j][i] = sp.optimize.curve_fit(lambda t, b, c: b * np.exp(-c * t), n_meas, np.average(1 - fid_data[j, i, :, :], axis=1))[0]
        coef_std[j][i] = np.sqrt(np.diag(sp.optimize.curve_fit(lambda t, b, c: b * np.exp(-c * t), n_meas, np.average(1 - fid_data[j, i, :, :], axis=1))[1]))

m_s = 6
l_w = 2
c_meas = ['red', 'orange', 'green', 'blue']
