...
def function(ind_r, n_m, meas):
    start = time.time()
    rho = r[ind_r]
    if meas == 'pauli': O = POVM_paulibasis(n_m, p, dim) # create M POMVs
    if meas == 'rand': O = POVM_randbasis(n_m, p, dim) # create M POMVs
    if meas == 'rand2': O = POVM_randbasis_2meas(n_m, p, dim) # create M POMVs
    x = experiment(O, rho)
...
for idm, n_m in enumerate(M):
    for idr in range(rep_O):
        ...
        out += [Parallel(n_jobs=cores)(delayed(function)(ind_r, n_m, 'pauli') for ind_r in np.arange(L))]
        out += [Parallel(n_jobs=cores)(delayed(function)(ind_r, n_m, 'rand') for ind_r in np.arange(L))]
        out += [Parallel(n_jobs=cores)(delayed(function)(ind_r, n_m, 'rand2') for ind_r in np.arange(L))]
